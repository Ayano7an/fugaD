...
| 词后的 `(N)` | 该词的累计遭遇次数，与网页 LESEN 页右栏是同一个数字 |
...
真正的词频存在 `index.html` 的 localStorage（`vocabStore`，按旬分桶），扩展跨源读不到。
所以主应用会把一份扁平的 `词 → 总次数` 表推给伺服器（`POST /api/freq`，防抖 3 秒），
`GET /api/lookup` 再把它读回来。

两边靠这个规则保持一致：**应用推送时是覆盖而非累加**。扩展查一次词，伺服器
同时入队并把快取 +1；主应用下次打开取走队列、词频自然增加，再推送覆盖掉伺服器的值。
所以即使主应用关着，面板上的数字也是对的，而不会随时间漂移。

关掉「计入频率统计」后，查词不再计次，`(N)` 仍会显示当前值。
